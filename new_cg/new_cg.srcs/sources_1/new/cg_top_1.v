...
always@(posedge clk)
 begin
   ...
   if(o_valid_3 &  !final_reg & count & end_flag_6 & check_op_3)
   begin
...
always@(posedge clk)    /// u block
 begin
   ...
   begin
      mem_u[wr_addr_7]<=o_data_3;
	  //wr_addr_7<=wr_addr_7+1;
      
   end
  else if(o_valid_3 &  !final_reg & count & end_flag_6 & check_op_3 )
    begin
      mem_u[wr_addr_7]<=o_data_3;
	  //wr_addr_7<=wr_addr_7+1;
    end		
...
always@(posedge clk)    /// m block
 begin
   if(!final_reg & !count & o_valid_mat_mult & !flag_3)     //w=A*u
    begin
	 mem_m[wr_addr_2]<=o_data_mat_mult;	   
	   //wea_m<=1'b1;
	   //addra_m<=wr_addr_2;
	end
  else if(o_valid_4 & !final_reg & !count)
   begin
       mem_m[wr_addr_8]<=o_data_4;
	  //wea_m<=1'b1;     
	 // addra_m<=wr_addr_8;
	  //wr_addr_8<=wr_addr_8+1;
   end
 ...
 begin
   if(!final_reg & !count & o_valid_mat_mult & !flag_3)   //w=A*u
    begin
	   mem_s[wr_addr_2]<=o_data_mat_mult;
	  // wea_s<=1'b1;
	   //addra_s<=wr_addr_2;
	end
  else if(o_valid_2 & !final_reg & count & !end_flag_4 & !check_op_2)
    begin
	  mem_s[wr_addr_s]<=o_data_2;
	  //wea_s<=1'b1;
	  //addra_s<=wr_addr_s;
	  //wr_addr_s<=wr_addr_s+1;
   end 
